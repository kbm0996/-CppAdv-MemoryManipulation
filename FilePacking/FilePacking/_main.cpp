...
void main(void)
{
	...
	while (1)
	{
		...
		case 1:
		{
			...
			{
				szNameArray[iCnt] = (WCHAR*)malloc(sizeof(st_LOCAL_FILE_HEADER::szFileName));
				memset(szNameArray[iCnt], 0, sizeof(st_LOCAL_FILE_HEADER::szFileName));

				wprintf(L"Input %d File Name : ", iCnt);
				wscanf_s(L"%s", szNameArray[iCnt], (unsigned)_countof(st_LOCAL_FILE_HEADER::szFileName));
			}
...
void FilePacking(WCHAR * szPackingFile, int iElementFileCnt, WCHAR ** szElementNameArray)
{
	...
	st_CENTRAL_FILE_HEADER PackHeader;
	...
	// 3. write st_CENTRAL_FILE_HEADER
	int iMaxFile = 0;
	st_LOCAL_FILE_HEADER LocalHeader;
	fwrite(&PackHeader, sizeof(st_CENTRAL_FILE_HEADER), 1, pDestFile);
	for (int iCnt = 0; iCnt < iElementFileCnt; ++iCnt)
	{
		fseek(pSourceFile[iCnt], 0, SEEK_END); // file size
		LocalHeader.iFileSize = ftell(pSourceFile[iCnt]);
		wcscpy_s(LocalHeader.szFileName, szElementNameArray[iCnt]);

		fwrite(&LocalHeader, sizeof(LocalHeader), 1, pDestFile);

		if (iMaxFile < LocalHeader.iFileSize)
			iMaxFile = LocalHeader.iFileSize;
	}
...
void FileUnpacking(WCHAR * szPackingFile)
{
	...
	// 1. Header Check
	st_CENTRAL_FILE_HEADER PackHeader;
	size_t iReadPos = fread(&PackHeader, sizeof(st_CENTRAL_FILE_HEADER), 1, pDestFile);
	if (iReadPos != 1)
	{
	...
	iReadPos = sizeof(st_LOCAL_FILE_HEADER) * PackHeader.iFileCnt;
	st_LOCAL_FILE_HEADER * LocalHeader = (st_LOCAL_FILE_HEADER*)malloc(iReadPos);
	memset(LocalHeader, 0, iReadPos);

	iReadPos = fread(LocalHeader, iReadPos, 1, pDestFile);
	if (iReadPos != 1) {
	...
	{
		if (iMaxSize < LocalHeader[iCnt].iFileSize)
			iMaxSize = LocalHeader[iCnt].iFileSize;
	}
	...
	for (int iCnt = 0; iCnt < PackHeader.iFileCnt; ++iCnt)
	{
		wprintf(L"`%s` Size : %d \n", LocalHeader[iCnt].szFileName, LocalHeader[iCnt].iFileSize);

		FILE* pSourceFile;
		if (_wfopen_s(&pSourceFile, LocalHeader[iCnt].szFileName, L"wb") != 0)
		{
			wprintf(L"Can't Open Source File `%s`\n", LocalHeader[iCnt].szFileName);
		}
		fread(pBuffer, LocalHeader[iCnt].iFileSize, 1, pDestFile);
		fwrite(pBuffer, LocalHeader[iCnt].iFileSize, 1, pSourceFile);
		fclose(pSourceFile);
	}
